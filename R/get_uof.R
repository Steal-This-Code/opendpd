...
get_uof <- function(year, start_date = NULL, end_date = NULL,
  ...
  # --- Year to Resource ID Mapping (Restricted to 2017-2020) --- *** UPDATED ***
  ...
  # --- Updated Input Validation for Year --- *** UPDATED ***
  ...
  # Determine correct field names based on year (within 2017-2020 range)
  if (year >= 2020) { # Specifically year 2020 in the current map
    date_field <- "occurred_dt"
    service_type_field <- "service_type"
  } else { # Years 2017, 2018, 2019
  ...
  if (is.null(where)) {
    ...
    if (!is.null(force_type)) { # Field: forcetype
      if(!is.character(force_type)) stop("`force_type` must be char.", call. = FALSE)
      clause <- sql_in_clause("forcetype", force_type)
      if (!is.null(clause)) all_where_clauses <- c(all_where_clauses, clause)
    }
    if (!is.null(reason)) { # Field: uof_reason
      if(!is.character(reason)) stop("`reason` must be char.", call. = FALSE)
      clause <- sql_in_clause("uof_reason", reason)
      if (!is.null(clause)) all_where_clauses <- c(all_where_clauses, clause)
    }
    if (!is.null(service_type)) { # Field: service_ty or service_type
      if(!is.character(service_type)) stop("`service_type` must be char.", call. = FALSE)
  ...
  repeat {
    ...
    if(nchar(trimws(content_text)) <= 2) { data_chunk <- tibble::tibble() }
      ...
      tryCatch({ data_chunk <- jsonlite::fromJSON(content_text, flatten = TRUE); data_chunk <- dplyr::as_tibble(data_chunk) },
               error = function(e) { stop(paste0("Failed to parse JSON for year ", year,". Error: ", e$message), call. = FALSE) })
    }
  ...
  return(final_data) # Returns a tibble, schema may vary by year
}
