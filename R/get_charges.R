...
get_charges <- function(start_date = NULL, end_date = NULL,
  ...
  base_url <- "https://www.dallasopendata.com/resource/9u3q-af6p.json" # *** UPDATED ENDPOINT ***
  ua <- httr::user_agent("dallasopendata/0.1.0 (https://github.com/galvanthony/dallasopendata)") # Customize!
  query_params <- list(...)

  # --- Filtering Logic ---
  all_where_clauses <- list()
  # (Using the corrected helper function from previous step)
  ...
  if (is.null(where)) {
    # -- Date filtering (using 'arrestdate' text field) -- *** UPDATED FIELD & TYPE ***
    # NOTE: Filtering assumes text dates are in sortable YYYY-MM-DD format
    if (!is.null(start_date)) {
      tryCatch({
        # Format date for comparison, time part might be ignored by API if field is just date text
        start_date_fmt <- format(as.Date(start_date), "%Y-%m-%d") # Use only date part?
        # Or include T00:00:00 if source text includes time? Test required. Using date only first.
    ...
    # -- Charge Filters -- *** NEW FILTERS ***
    if (!is.null(charge_description)) { # Field: chargedesc (Text)
      if(!is.character(charge_description)) stop("`charge_description` must be char.", call. = FALSE)
    ...
    if (!is.null(severity)) { # Field: severity (Text)
      if(!is.character(severity)) stop("`severity` must be char.", call. = FALSE)
      clause <- sql_in_clause("severity", severity)
      if (!is.null(clause)) all_where_clauses <- c(all_where_clauses, clause)
    }
    if (!is.null(penalty_class)) { # Field: pclass (Text)
      if(!is.character(penalty_class)) stop("`penalty_class` must be char.", call. = FALSE)
      clause <- sql_in_clause("pclass", penalty_class)
      if (!is.null(clause)) all_where_clauses <- c(all_where_clauses, clause)
    }
    if (!is.null(statute)) { # Field: statute (Text)
      if(!is.character(statute)) stop("`statute` must be char.", call. = FALSE)
      clause <- sql_in_clause("statute", statute)
      if (!is.null(clause)) all_where_clauses <- c(all_where_clauses, clause)
    }

    # -- NIBRS Filters -- *** RE-ADDED ***
    ...
    # -- Geography & GeoConversion Filters Removed --
  ...
  repeat {
    ...
    # Fewer field name issues expected with this simpler dataset? Keep basic check.
