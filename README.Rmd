...
The goal of opendpd is to make Dallas Police open data easier to download, preserve, standardize, and query safely when the source portal is inconsistent.
...
The package provides functions starting with `get_` to retrieve data from different datasets. By default, they now download raw data for the requested date range, clean and normalize it, and then apply dedicated filters locally. This is slower than portal-side filtering, but it is more robust when raw values are inconsistent. Use `filter_strategy = "portal"` if you want the faster, older behavior, and use `download_dpd_raw()` when you want the untouched source data.
...
Functions like `standardize_division()` and `standardize_district()` are still available when you want to overwrite a specific column with a factor after inspection, but most workflows can now rely on the helper columns created by `clean_*()` and the default `get_*()` pipeline.
